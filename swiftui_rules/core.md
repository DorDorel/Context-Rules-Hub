...
# Agentic Skill Protocol (Meta-Instructions)

You are operating within an **Agentic Skill Architecture**. Your capabilities are modularized into "Skills".
You must adhere to the **Progressive Disclosure** principle when using these skills.

## The Skill Structure

When you identify a relevant skill (e.g., in `.github/skills/` or `.gemini/skills/`), you must treat it as a structured package, not just a file.

1.  **SKILL.md (The Entry Point):**
    - This is the ONLY file you read initially.
    - It contains the **Workflow** and **Decision Logic**.
    - Do NOT hallucinate guidelines. If `SKILL.md` says "See references/x.md", you MUST read that file.

2.  **references/ (Deep Knowledge):**
    - Contains heavy documentation, schemas, or templates.
    - **Rule:** Only read files in `references/` if the specific task requires it. Do not pollute the context window with unnecessary references.

3.  **scripts/ (Executable Tools):**
    - Contains Python/Bash scripts for deterministic tasks.
    - **Rule:** If a script exists for a task (e.g., `scripts/generate_view.py`), PREFER running it over writing code manually.

## Execution Strategy

1.  **Detect Intent:** Map the user's request to a specific Skill Name.
2.  **Load Context:** Read the `SKILL.md` of that skill.
3.  **Expand (If needed):** If the task is complex, look for `references/` inside the skill folder.
4.  **Execute:** Generate the code or run the script based strictly on the loaded context.

> **Prime Directive:** You are a specialized agent. Do not rely on your general training data when a Skill provides specific, domain-expert instructions. The Skill is the Source of Truth.
